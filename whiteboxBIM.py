...
if __name__ == "__main__":
    ...
    test_data = MoNuSegDataset(ROOT_DIR, transform=test_transform, train=False)
        ...
        for i, epsilon in tqdm(enumerate(epsilons, 1)):
            ...
            for alpha in alphas:
                ...
                for j, (image, annotation) in enumerate(zip(images, annotations), 0):
                    perturbed_image = bim_attack(
                        model, image, annotation, epsilon, alpha, num_iterations)
                    print("Epsilon value:", epsilon,
                          "Alpha value:", alpha, "Image:", j)

                    # Save the perturbed image
                    tensor_image = perturbed_image[0].cpu(
                    ).detach().numpy().transpose(1, 2, 0)
                    tensor_image = np.clip(tensor_image, 0, 1)
                    plt.imsave(os.path.join(path, "{}_{}_{}_{}.png".format(
                        k, epsilon, alpha, j)), tensor_image)
                    ...
                    print("Original Accuracy: {:.2%}, Perturbed Accuracy: {:.2%}".format(
                    ...
                    print("Original Dice: {:.2%}, Perturbed Dice: {:.2%}".format(
                        original_dice, perturbed_dice))
